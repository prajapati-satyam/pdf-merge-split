...
async function merge_pdf(pdfpaths: any[]) {
  ...
  for (let i = 0; i < existingPdf.length; i++) {
    const copyPages = await final_pdf.copyPages(
      existingPdf[i],
      existingPdf[i].getPageIndices()
    );

    copyPages.forEach((page: any) => {
...
async function split_pdf(inputpath: string, option: any) {
  if (
    !inputpath ||
  ...
  if (option.mode === "range") {
    if (
      option.pageNumbers.length !== 2 ||
      option.pageNumbers[0] > option.pageNumbers[1] ||
      !option.pageNumbers
    ) {
      console.log(
        new Error(
          "Check rangeNumbers values, first value can't be Greter than second, must be exact two elements in rangeNumbers, Values must be Number, Check Your rangeNumbers option"
        )
      );
    ...
    let page_count2: number[] = [];
    let finalAllPdfBytes: any[] = [];
    ...
    if (page_count2.length > existingPdf.getPageCount()) {
      console.log(
        new Error("Input Range Can't exceed Total Number of Pages in PDF")
      );
  ...
  if (option.mode === "pagenumber") {
    ...
    for (let i = 0; i < option.pageNumbers.length; i++) {
      if (option.pageNumbers[i] > existingPdf.getPageCount()) {
        console.log(
          new Error("Page Numbers can't exceed Tottalpages of PDF")
        );
